...
export default function ViewMotorPolicyPage() {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">Motor Policy Details</h1>
        </div>
        <div className="flex items-center gap-2">
          <Button asChild variant="outline"><Link href="/motor"><ArrowLeft className="h-4 w-4 mr-2"/>Back</Link></Button>
          {policy && (
            <Button asChild><Link href={`/motor/${policy.id}/edit`}><Pencil className="h-4 w-4 mr-2"/> Edit</Link></Button>
          )}
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Individual motor insurance policy</CardDescription>
        </CardHeader>
        <CardContent>
          {!policy ? (
