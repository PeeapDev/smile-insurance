...
export default function CompanyClaimsPage() {
  ...
  const org = (params?.company || "org").toString()
  ...
  const filtered = useMemo(() => {
    ...
    return base.filter((c) =>
      (filterStatus === "all" || c.status === (filterStatus as any)) &&
      (filterType === "all" || (c.type || "").toLowerCase() === filterType.toLowerCase()) &&
      (`${c.id} ${c.claimant}`.toLowerCase().includes(q))
    )
  }, [claims, query, filterStatus, filterType])
